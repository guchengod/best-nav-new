...
async function fetchAPI<T>(endpoint: string, options: RequestInit = {}, requiresAuth: boolean = false): Promise<T> {
    ...
    // 如果是 FormData，不要设置 Content-Type，让浏览器自动设置 boundary
...
export const api = {
    ...
    // 菜单管理
    async menus(): Promise<MenuItemTree[]> {
        const response = await fetchAPI<{data: MenuItemTree[]}>('/api/menus');
        return response.data;
    },
    ...
    async fetchIcon(url: string): Promise<{ url: string }> {
        return fetchAPI('/api/fetch-icon', {
            method: 'POST',
