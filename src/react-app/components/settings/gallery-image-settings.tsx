...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from "@/components/ui/dialog"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import { Pencil, Trash2, Upload, Loader2, Download, Share2, X } from "lucide-react"
import { api } from '@/lib/api'
import { GalleryImage, GalleryCategory } from '@/types/settings'
import { toast } from "@/hooks/use-toast"
import { Label } from '@/components/ui/label'
...
export function GalleryImageSettings() {
    ...
    const handleShare = (url: string) => {
        const sharePath = url.replace('/api/rfile/', '/api/share/img/');
    ...
    return (
        <div className="space-y-4 h-[80vh] flex flex-col">
            ...
            >
                ...
                <div className="flex flex-col items-center gap-2">
                    <Upload className="h-8 w-8 text-muted-foreground" />
                    <p className="text-sm text-muted-foreground">拖拽图片到此处，或 <span onClick={() => fileInputRef.current?.click()} className="text-primary cursor-pointer hover:underline">点击选择</span></p>
                </div>
