...
import {ScrollToTop} from "./scroll-to-top";
import { ViewProvider, useView } from "@/components/ui/view-context";
import { NavigationProvider } from "@/context/navigation-context"; // 导入 NavigationProvider
import Content from "./content";
import NoteContent from "./note-content";
import NoteSidebar from "./note-sidebar";
import { GalleryContent } from "./gallery-content";
            ...
            try {
                ...
                if (settings) {
                    ...
                    if (settings.data.theme) {
                        setTheme(settings.data.theme as any);
                    }
                    // 应用语言
                    if (settings.data.language) {
                        i18n.changeLanguage(settings.data.language);
                    }
                }
            } catch (error) {
                console.error("Failed to load system settings:", error);
